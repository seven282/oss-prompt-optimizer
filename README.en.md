...
| `outputStyle` | `'plain'` \| `'role-task-goal'` \| `'sections'` | `'plain'` | Heading-free continuous prose (default, fewer tokens), three parseable labels (`角色：/任务：/目标：` or `Role:/Task:/Goal:` for downstream parsing into role / task / goal; the goal line merges background constraints and the output spec), or four-section headings (`## Role`/`## Task`/`## Context`/`## Format` — also the internal reference frame during optimization) |
...
| `maxTokenRetryFactor` | number 1–3 | `1.5` | Jump-expansion multiplier when the output hits `maxTokens` (1200→1800→2700…); expansion does not consume the retry budget and resumes from the truncated prefix; `1` disables |
...
| `maxTotalTokens` | int ≥0 | `20000` | Cumulative token ceiling across all calls of ONE optimization (system prompts + generated text, plugin heuristic estimate); once reached, expansion jumps and retries stop and the best result degrades as usual (`BUDGET_EXCEEDED`); `0` disables. Complements `maxCalls` (call-count bound) and `maxTokensCap` (per-call output bound) |
...
| `dreamInsightFeedback` | boolean | `false` | Cross-turn dream-insight feedback: when on, the latest `senseNeeds` appendix of this session is injected into later optimize/iterate calls (marked AI-inferred, non-fact; session-scoped, 30-min TTL; stored truncated to 200 chars against growth) |
| `senseNeedsSeparate` | boolean | `false` | D6 experimental tier: appendix via a separate light call — the main line optimizes normally without the sensing block (full localTemplate support, on mode can produce an appendix too), then a second maxTokens=250 call emits only the `--- Extended insights ---` appendix (silent failure, body returned as-is). Adds one call per dream run, but the main output is shorter and steadier and the budget-expansion amplifier is eliminated; off by default (single inline call) |
...
| `contextMaxMessages` | int 0–100 | `10` | Max recent messages gathered as context when `contextAware` is on; `0` disables |
...
- The meta-prompt carries `{{原始指令}}`-style placeholders substituted at runtime, the instruction-is-data injection guardrail, the language rule (`{{语言规则}}`), a no-code-fence rule, a terseness requirement and a pre-output self-check; the output structure switches between the heading-free, three-label, and four-section structure blocks via `outputStyle`.
