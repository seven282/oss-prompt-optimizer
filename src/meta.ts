...
- ## Role：为执行提示词的主体设定具体角色，采用「身份＋能力＋行为」三要素写法——身份（如"资深工程师"）不必以"你是"开头，能力（如"精通 Python、擅长可视化"）与行为约束（如"先给结论、拒绝猜测"）同样合格且往往更可执行；角色必须与任务强相关——原始指令已明确执行主体时沿用，否则按任务类型与领域推断（代码任务对应资深工程师、文案对应资深撰稿人），并体现所需专业度；避免"AI 助手"这类空泛角色。
- ## Task：用明确的动词描述要完成的任务，必要时拆解为可执行的步骤；目标要具体、可衡量；说明完成标准（做到什么程度算完成）。
- ## Context：补充背景、约束条件、目标受众与质量标准；信息可来自原始指令或对话上下文，不得虚构新事实，原始指令已含的信息不必重复；信息不足时显式声明假设。
- ## Format：规定输出的结构、格式、长度与风格；原始指令中的格式与长度要求必须保留；结构、格式、长度与风格四项须齐全，原始指令未明确的给合理默认。`
...
const SELFCHECK_SECTIONS = `- 输出前自查：四个段落标题必须全部存在且每段有实质内容；角色与任务强相关、不空泛，且含能力或行为描述（仅一句空身份不算合格）；Context 无虚构事实；Format 覆盖结构、格式、长度与风格四项。缺一不可。`

/** Plain-mode pre-output self-check. */
const SELFCHECK_PLAIN = `- 输出前自查：正文完整覆盖上述四个方面（含完成标准、假设声明与格式默认值），长度足以直接执行，且不含任何小节标题（## 等）或字段标签；角色部分需含能力或行为描述，仅一句空泛身份不算合格。`
...
- ## Context: add background, constraints, target audience, and quality standards; facts may come from the raw instruction or the conversation context — never invent new ones and do not repeat what the raw instruction already states; when information is missing, state assumptions explicitly.
- ## Format: specify the structure, format, length, and style of the output; keep any format and length requirements from the raw instruction; cover all four aspects, giving reasonable defaults for any the raw instruction does not specify.`
